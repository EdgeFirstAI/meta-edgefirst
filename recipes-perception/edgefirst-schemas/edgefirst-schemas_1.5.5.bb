...
do_install() {
    ...
    install -d ${D}${libdir}
    install -m 0755 ${SRCDIR}/edgefirst-schemas-linux_${TARGET_ARCH}-${PV}/lib/libedgefirst_schemas.so.${PV} ${D}${libdir}/
    ...
    install -m 0644 ${SRCDIR}/edgefirst-schemas-linux_${TARGET_ARCH}-${PV}/lib/libedgefirst_schemas.a ${D}${libdir}/

    # Install pkg-config file
    install -d ${D}${libdir}/pkgconfig
    install -m 0644 ${SRCDIR}/edgefirst-schemas-linux_${TARGET_ARCH}-${PV}/lib/pkgconfig/edgefirst-schemas.pc ${D}${libdir}/pkgconfig/

    # Install headers
    install -d ${D}${includedir}/edgefirst
    install -m 0644 ${SRCDIR}/edgefirst-schemas-linux_${TARGET_ARCH}-${PV}/include/edgefirst/schemas.h ${D}${includedir}/edgefirst/

    # Install Python wheel
    install -d ${D}${PYTHON_SITEPACKAGES_DIR}
    unzip ${SRCDIR}/edgefirst_schemas-${PV}-py3-none-any.whl -d ${D}${PYTHON_SITEPACKAGES_DIR}
}
